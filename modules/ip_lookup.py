...
def fetch_rapiddns(ip):
    ...
    headers = {"User-Agent": random.choice(USER_AGENTS), **EXTRA_HEADERS}
...
def Ip_lockup_menu():
    ...
    if not output_file:
        output_file = "ip_lookup_results.txt"
    ...
    while True:
        threads_input = get_input(Fore.CYAN + " ➜  Enter the number of threads to use (1-50): ").strip()
        if threads_input.isdigit():
            threads = int(threads_input)
            if 1 <= threads <= 50:
                break
            else:
                print(Fore.RED + "⚠️ Please enter a number between 1 and 50.")
        else:
